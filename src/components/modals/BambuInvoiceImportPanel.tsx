...
import { parseBambuInvoice, extractRawDebug } from '../../lib/parseBambuInvoice';
...
export default function BambuInvoiceImportPanel({ onImport, onClose }: Props) {
  type PanelState = 'idle' | 'parsing' | 'preview' | 'error' | 'debug';
  ...
  async function handleFile(file: File) {
    ...
    if (debugMode) {
      const result = await extractRawDebug(file);
      if (result.error) {
        setErrorMsg(result.error);
        setPanelState('error');
        return;
      }
      setDebugText(result.rowsView.join('\n'));
          ...
          <label className="flex items-center gap-2 cursor-pointer select-none">
            <div
              ...
              className={[
                ...
                debugMode ? 'bg-amber-400' : 'bg-slate-200',
              ].join(' ')}
            >
              <span
                className={[
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <p className="text-xs font-semibold text-amber-700 bg-amber-50 border border-amber-200 px-3 py-1.5 rounded-lg">
            Debug mode — {debugPages} page{debugPages !== 1 ? 's' : ''} extracted
          </p>
          <div className="flex items-center gap-2">
            <button
              onClick={copyToClipboard}
              className="text-xs font-medium text-white bg-[#1e2a3a] hover:bg-[#2a3a4f] px-3 py-1.5 rounded-lg transition-colors"
            >
              {copied ? '✓ Copied!' : 'Copy all'}
            </button>
            <button
              onClick={() => { setPanelState('idle'); setDebugText(''); }}
        ...
        <p className="text-xs text-slate-500">
          Each row shows <code className="bg-slate-100 px-1 rounded">y=NNN</code> (vertical position) and the raw text columns
          the parser sees. Copy this and paste it back to share the PDF structure.
        </p>

        <textarea
          readOnly
          value={debugText}
          className="w-full h-96 text-[10px] font-mono bg-slate-900 text-green-300 rounded-xl p-3 resize-none focus:outline-none border border-slate-700"
          spellCheck={false}
        ...
        <div className="flex gap-3">
          <button
            onClick={() => { setPanelState('idle'); setDebugText(''); setDebugMode(false); }}
